...
if(! function_exists('sendTelegramMsgDeposit')){
    function sendTelegramMsgDeposit($id, $amount)
    {
       /* $users      = \App\User::where('id', $id)->first();
        $user_game  = \App\GameMember::where('AccountUniqueid', $users->account_id)->first();
        $user_nick  = ($user_game === null) ? '' : $user_game->PlayerName;
        ...
        foreach ($telegrams as $telegram)
        {
            $arr = [
                ...
                'user_nick' => $user_nick,
                'admin_telegram_id' => $telegram->telegram_id,
            ];
